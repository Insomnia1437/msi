...
static int substituteGetNextSet(void *pvt, char **filename)
{
    ...
    *filename = 0;
    while (psubFile->token == tokenSeparater)
        subGetNextToken(psubFile);
    ...
    *filename = psubInfo->filename;
    ...
    if (psubFile->token == tokenRBrace)
        return (0);
    if (psubFile->token != tokenString || strcmp(psubFile->string, "pattern") != 0)
    {
...
{
    /* process the line like:
    {USER = "sdcswd", HOST = "atari"} # trail comment
    */
    int pos;
    char *pch;
    pch = strchr(str, '#');
...
static char *subGetNextLine(subFile *psubFile)
{
    ...
    trimcomment(psubFile->inputBuffer);
    psubFile->pnextChar = &psubFile->inputBuffer[0];
